let ShowToCompleteDeliveryFunc = async () => {
    ...
    let LocalIronedData = _.filter(LocalCollectionData, (LoopItem) => {
        if ("IroningDone" in LoopItem.value) {
            return LoopItem.value.IroningDone.KTF === true;
        };
    });

    let LocalFilteredData = _.filter(LocalIronedData, (LoopItem) => {
        if ("DeliveryDone" in LoopItem.value) {
            return (LoopItem.value.DeliveryDone.KTF === true) === false;
        } else {
            return true;
        }
    });
